...
st.caption("Builds a fresh watchlist (<= max buy) then scans for edge. Uses Playwright.")
...
with st.sidebar:
    ...
    headed = st.checkbox("Headed mode (debug)", value=False)
...
def run_cmd(args: list[str]) -> tuple[int, str]:
    """Run python -m keyflip.main ... and return (code, combined_output)."""
    cmd = ["python", "-m", "keyflip.main"] + args
    p = subprocess.run(cmd, cwd=str(ROOT), capture_output=True, text=True)
    out = (p.stdout or "") + ("\n" + (p.stderr or "") if p.stderr else "")
    return p.returncode, out

if clear_cache:
    code, out = run_cmd(["--clear-cache"])
    st.code(out)
    st.stop()

if clear_recent:
    code, out = run_cmd(["--clear-recent"])
    st.code(out)
    st.stop()
...
if allow_eur:
    args_common += ["--allow-eur", "--eur-to-gbp", str(float(eur_to_gbp))]
if headed:
    args_common += ["--headed"]
if debug:
    args_common += ["--debug"]
...
with tabs[1]:
    if SCANS.exists():
        # show last batch by timestamp if present
