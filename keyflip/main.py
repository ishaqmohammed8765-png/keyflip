...
def _build_parser() -> argparse.ArgumentParser:
    ...
    # Currency (your core supports these even though current Fanatical/Eneba readers may only return GBP)
    ...
    # Diagnostics (THIS is what helps you fix “0 links / no build” fast)
...
def _run_diag_harvest(pages_per_source: int) -> int:
    ...
    for name, src_url in FANATICAL_SOURCES.items():
        try:
            ...
            total += len(uniq)
            sample = uniq[:5]
            log.info("source=%s links=%d unique=%d sample=%s", name, len(links), len(uniq), sample)
        except Exception:
            log.exception("DIAG harvest failed for source=%s url=%s", name, src_url)
    log.info("DIAG: total unique harvested across sources (not deduped across sources): %d", total)
    return 0


def _run_diag_price(pages_per_source: int, n: int) -> int:
    if n <= 0:
        return 0
    log.info("DIAG: Price-reading %d sampled Fanatical URLs", n)

    # Build a small pool of harvested URLs
    pool: list[str] = []
...
def main(argv: list[str] | None = None) -> int:
    ...
    if int(args.diag_price) > 0:
        return _run_diag_price(pages_per_source=int(args.pages_per_source), n=int(args.diag_price))

    # Normalize verify behavior
    verify_safety_cap = int(args.verify_safety_cap) if int(args.verify_safety_cap) > 0 else 14
    ...
    if do_build:
        ...
        if do_scan and (dfw is None or getattr(dfw, "empty", True)):
            ...
            log.warning("Next step: run --diag-harvest (and maybe --diag-price 5) to see why.")
            return 0
