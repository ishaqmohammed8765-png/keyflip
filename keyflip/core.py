    ...
    with FanaticalPWClient(headless=True) as fan:
        ...
        for url in ordered_pool:
            ...
            try:
                ...
                debug_rows.append(
                    {"url": url, "title": "", "price": "", "decision": "error_get_buy", "notes": str(e)}
                )
                log.exception("get_buy failed: %s", e)
                continue
...
def scan_watchlist(
    ...
    - DOES NOT stop after 1 item if one item errors (per-row try/except)
    - scan_limit counts ATTEMPTS (not just rows appended)
    """
    ...
    log.info(
        "Scan settings: scan_limit=%s avoid_recent_days=%s run_budget_s=%s item_budget_s=%s watchlist=%s",
        scan_limit,
        avoid_days,
        run_budget,
        item_budget,
    ...
    rows: List[Dict[str, object]] = []
    attempted = 0  # IMPORTANT: counts attempts, so scan_limit behaves predictably

    with FanaticalPWClient(headless=True) as fan:
        for _, r in watch2.iterrows():
            if not _budget_ok(start, run_budget):
                log.info("Stopping scan: run budget reached.")
                break

            # scan_limit: 0 = unlimited
            if scan_limit > 0 and attempted >= scan_limit:
                log.info("Stopping scan: scan_limit reached (%d).", scan_limit)
                break
            ...
            try:
                ...
                # If item budget exceeded, skip RECORDING (but keep scanning next items)
                if item_budget > 0 and elapsed > item_budget:
                    log.info("Skip (item budget %.2fs) title=%s elapsed=%.2f", item_budget, title, elapsed)
                    continue
                ...
                # CRITICAL FIX: don't let one failure stop the whole scan
                log.exception("Scan failed for title=%r url=%s err=%s", title, buy_url, e)
                continue
    ...
    log.info(
        "Scan complete: attempted=%d recorded=%d passes=%d scans=%s passes=%s",
        attempted,
        len(batch),
        passes_count,
        scans_path,
