...
def test_blocked_buy_fallback_defaults_to_craigslist_for_uk_locale(monkeypatch) -> None:
    ...
    client = EbayClient(RunSettings(marketplace="ebay"))
    assert client._blocked_buy_fallback_marketplaces() == ["craigslist", "mercari"]


def test_blocked_buy_fallback_relaxes_delivery_only_for_craigslist(monkeypatch) -> None:
    client = EbayClient(RunSettings(marketplace="ebay", delivery_only=True))
    target = Target(id=1, name="Nintendo Switch", query="Nintendo Switch OLED")
    ...
    fallback_listing = Listing(
        ebay_item_id="craigslist-1",
        target_id=1,
        title="Nintendo Switch OLED",
        url="https://london.craigslist.org/vgm/d/london-nintendo-switch-oled/1234567890.html",
        price_gbp=180.0,
        shipping_gbp=0.0,
        total_buy_gbp=180.0,
        listing_type="fixed",
        raw_json={"source": "craigslist_html", "shipping_type": "pickup"},
    )
    fallback_ok = SearchResult(
        listings=[fallback_listing],
        retry_report=[],
    ...
    observed_delivery_only: list[bool] = []

    def fake_search_with_retry(self, _target, mode):  # noqa: ANN001
        if self.settings.marketplace == "ebay":
            return blocked
        observed_delivery_only.append(self.settings.delivery_only)
        return fallback_ok

    monkeypatch.setattr(EbayClient, "_search_active_with_retry", fake_search_with_retry)
    monkeypatch.setenv("BUY_BLOCKED_FALLBACK_ENABLED", "1")
    monkeypatch.setenv("BUY_BLOCKED_FALLBACK_MARKETPLACE", "craigslist")

    result = client.search_active_listings(target)

    assert result.status == "ok"
    assert observed_delivery_only == [False]
    assert len(result.listings) == 1
    assert result.listings[0].raw_json.get("source") == "craigslist_html"
